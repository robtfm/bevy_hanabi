//! Example of using the circle spawner with random velocity, will fail to emit
//! intermitently.
//!
...
const DEMO_DESC: &str = include_str!("faulty_emission.txt");

fn main() -> Result<(), Box<dyn std::error::Error>> {
    let app_exit = utils::DemoApp::new("faulty_emission")
        .with_desc(DEMO_DESC)
        .build()
...
fn setup(
    ...
    let effect = effects.add(
        EffectAsset::new(
            32768,
            SpawnerSettings::rate(32.0.into()).with_probability(0.1),
            module,
        )
        .with_name("faulty_emission")
        .init(init_pos)
        .init(init_vel)
        .init(init_age)
        .init(init_lifetime)
