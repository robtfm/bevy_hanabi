//! Example of using the circle spawner with random velocity, that waits
//! a few seconds before the first emission.
//!
...
const DEMO_DESC: &str = include_str!("delayed_start.txt");

fn main() -> Result<(), Box<dyn std::error::Error>> {
    let app_exit = utils::DemoApp::new("delayed_start")
        .with_desc(DEMO_DESC)
        .build()
...
fn setup(
    ...
    let effect = effects.add(
        EffectAsset::new(
            32768,
            SpawnerSettings::burst(32.0.into(), 1.0.into()).with_first_emission(5.),
            module,
        )
        .with_name("circle")
        .init(init_pos)
